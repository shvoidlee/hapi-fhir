...
public class FhirSystemDao extends BaseFhirDao implements IFhirSystemDao {
    ...
    public Map<String, Long> getResourceCounts() {
        ...
        TypedQuery<Tuple> q = null;
        Map<String, Long> retVal = new HashMap<String, Long>();
        
        for (IFhirResourceDao<?> next : myResourceDaos) {
            String resourceName = next.getResourceType().getSimpleName();
            if (resourceName.equalsIgnoreCase("ListResource"))
                resourceName = "List";
            
            cq = builder.createTupleQuery();
            Root<ResourceTable> from = cq.from(ResourceTable.class);
            cq.multiselect(builder.count(from.get("myResourceType")).as(Long.class));
            cq.where(builder.equal(from.get("myResourceType"), resourceName));
            q = myEntityManager.createQuery(cq);
            for (Tuple result : q.getResultList()) {
                Long resourceCount = result.get(0, Long.class);
                ourLog.warn(resourceName + ":" + resourceCount);
                retVal.put(resourceName, resourceCount);
            }
        }
        return retVal;
    }
